...
class EsotericaManager:
    ...
    def _load_source(self, file_path):
        ...
        try:
            ...
            if "passages" in content:
                logger.debug(
                    f"Passages section found with keys: {list(content['passages'].keys())}"
                )
            ...
            self.sources[source_id] = {"meta": meta, "passages": content.get("passages", {})}
    ...
    def get_passage_for_card(self, card_id, source_id=None):
        ...
        if source_id and source_id in self.sources:
            ...
            return self._find_passage_in_source(
                self.sources[source_id], card_type, card_number, suit
            )
